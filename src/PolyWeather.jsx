...
function parseWeatherQuestion(question) {
  ...
  else if (threshold !== null && isHigh) type = 'temp_high';
  else if (threshold !== null) type = 'temp_high';
  return { type, threshold, unit, rainThreshold };
}
...
async function fetchPolyWeatherMarkets() {
  try {
    // Works locally via Vite proxy AND on Vercel via serverless function
    const params = new URLSearchParams({
...
function computeSignal(market, forecast, parsed) {
  if (!forecast || !parsed || parsed.type === 'unknown') return { marketProb: null, ourProb: null, edge: null, signal: 'INSUFFICIENT_DATA' };
  ...
  if (parsed.type === 'temp_high' && parsed.threshold !== null) {
    ...
    if (precips[0] > 0 && precipProb > 0.3) {
      const lambda = 1 / Math.max(precips[0], 0.01);
  ...
  return { marketProb, ourProb, edge, signal, confidence };
}

function SignalCard({ market, forecast, city }) {
  const parsed = useMemo(() => parseWeatherQuestion(market.question || ''), [market.question]);
  const signal = useMemo(() => computeSignal(market, forecast, parsed), [market, forecast, parsed]);

  const sigColor = signal.signal === 'BUY_YES' ? '#4ade80' : signal.signal === 'BUY_NO' ? '#f87171' : '#64748b';
  const sigLabel = signal.signal === 'BUY_YES' ? '▲ BUY YES' : signal.signal === 'BUY_NO' ? '▼ BUY NO' : '— HOLD';
  ...
  return (
    <div style={{
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'4px', flexWrap:'wrap' }}>
            <div style={{
              ...
              background:`${sigColor}20`, color:sigColor, border:`1px solid ${sigColor}40`, flexShrink:0,
            }}>{isBuy ? sigLabel : '— HOLD'}</div>
            {city && (
      ...
      {signal.marketProb !== null && signal.ourProb !== null && (
        <div style={{ marginBottom:'10px' }}>
          <div style={{ display:'flex', gap:'8px', marginBottom:'6px' }}>
            {[
              { label:'MARKET ODDS', val:`${(signal.marketProb*100).toFixed(0)}%`, color:'#e2e8f0' },
              { label:'NOAA MODEL',  val:`${(signal.ourProb*100).toFixed(0)}%`,   color:sigColor },
              { label:'EDGE',        val:`${signal.edge > 0 ? '+' : ''}${(signal.edge*100).toFixed(0)}%`, color:sigColor },
            ].map(({ label, val, color }) => (
          ...
          {isBuy && signal.confidence !== undefined && (
      ...
      <div style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
        {[{ label:'CLOSES', val:endDate }, { label:'VOLUME', val:volume }].map(({ label, val }) => (
          <div key={label}>
  ...
  return (
    <div style={{ marginBottom:'14px', background:'rgba(56,189,248,0.05)', borderRadius:'12px', padding:'10px 12px', border:'1px solid rgba(56,189,248,0.12)' }}>
      <div style={{ color:'#38bdf8', fontSize:'0.6rem', fontFamily:"'Courier New',monospace", letterSpacing:'0.1em', marginBottom:'8px' }}>
        📡 {city.name.toUpperCase()} — 7-DAY FORECAST
      </div>
        ...
        {(days.time || []).slice(0,7).map((date, i) => {
          ...
          return (
            <div key={date} style={{ flexShrink:0, textAlign:'center', minWidth:'44px', background:'rgba(255,255,255,0.03)', borderRadius:'8px', padding:'6px 4px' }}>
              <div style={{ color:'#475569', fontSize:'0.55rem', fontFamily:"'Courier New',monospace", marginBottom:'3px' }}>{label}</div>
              ...
              {prob > 20 && <div style={{ color:'#64748b', fontSize:'0.5rem', fontFamily:"'Courier New',monospace" }}>{prob}%💧</div>}
            </div>
          );
        })}
      </div>
...
export default function PolyWeather() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const weatherMarkets = rawMarkets.filter(m => {
        ...
        return /temperature|rain|snow|weather|high|low|°|degree|precip|storm/.test(q);
      });
      ...
      for (let i = 0; i < CITIES.length; i++) {
        ...
        } catch(e) { console.warn(`Forecast failed for ${CITIES[i].name}`, e); }
      }
  ...
  const enrichedMarkets = useMemo(() => markets.map(m => {
    ...
    const sig      = computeSignal(m, forecast, parsed);
  ...
  const displayMarkets = useMemo(() => {
    ...
    if (sortBy === 'edge')   filtered.sort((a,b) => Math.abs(b.sig.edge||0) - Math.abs(a.sig.edge||0));
    if (sortBy === 'volume') filtered.sort((a,b) => parseFloat(b.volumeNum||0) - parseFloat(a.volumeNum||0));
    if (sortBy === 'closes') filtered.sort((a,b) => new Date(a.endDate||0) - new Date(b.endDate||0));
    return filtered;
  ...
  const previewCity = selectedCity ? CITIES.find(c=>c.name===selectedCity) : CITIES[3]; // default Miami
    ...
    <div>
      ...
      {!loading && (
        <div style={{ display:'flex', gap:'8px', marginBottom:'12px' }}>
          ...
          <button onClick={loadData} style={{ flex:1, padding:'8px', borderRadius:'10px', border:'1px solid rgba(56,189,248,0.25)', background:'rgba(56,189,248,0.08)', color:'#38bdf8', fontSize:'0.62rem', fontFamily:"'Courier New',monospace", cursor:'pointer' }}>
            {loading ? '⟳ …' : '⟳ Refresh'}
          </button>
        </div>
      )}
      ...
      <div style={{ display:'flex', gap:'6px', marginBottom:'10px', flexWrap:'wrap' }}>
        ...
        <span style={{ color:'#334155', fontSize:'0.58rem', fontFamily:"'Courier New',monospace", alignSelf:'center', marginLeft:'4px' }}>SORT:</span>
        {[['edge','Edge'],['volume','Vol'],['closes','Date']].map(([val,label])=>(
      ...
      <div style={{ display:'flex', gap:'6px', marginBottom:'14px', flexWrap:'wrap' }}>
        <span style={{ color:'#334155', fontSize:'0.58rem', fontFamily:"'Courier New',monospace", alignSelf:'center' }}>CITY:</span>
      ...
      {loading && (
        <div style={{ textAlign:'center', padding:'40px 20px' }}>
          ...
          <div style={{ color:'#64748b', fontSize:'0.75rem', fontFamily:"'Courier New',monospace" }}>Fetching Polymarket weather markets…</div>
      ...
      {!loading && markets.length === 0 && (
        <div style={{ background:'rgba(245,158,11,0.07)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:'12px', padding:'14px', marginBottom:'14px' }}>
          <div style={{ color:'#f59e0b', fontWeight:'700', fontSize:'0.72rem', fontFamily:"'Courier New',monospace", marginBottom:'6px' }}>⚠ NO MARKETS FOUND</div>
          <div style={{ color:'#94a3b8', fontSize:'0.68rem', lineHeight:1.6 }}>
            Polymarket may have no active weather markets right now, or the API is temporarily unavailable. Forecast data below is still live.
          </div>
        </div>
      )}
        ...
        <div style={{ marginTop:'16px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px' }}>
            {CITIES.map(city => {
              ...
              const prob  = f.daily.precipitation_probability_max?.[0];
              return (
                <button key={city.name} onClick={()=>setSelectedCity(city.name===selectedCity?null:city.name)} style={{ background:selectedCity===city.name?'rgba(56,189,248,0.1)':'rgba(255,255,255,0.03)', borderRadius:'10px', border:`1px solid ${selectedCity===city.name?'rgba(56,189,248,0.3)':'rgba(255,255,255,0.06)'}`, padding:'10px', textAlign:'left', cursor:'pointer' }}>
                  ...
                  <div style={{ display:'flex', gap:'8px', alignItems:'baseline' }}>
                    ...
                    {prob > 20 && <span style={{ color:'#64748b', fontSize:'0.58rem' }}>💧{prob}%</span>}
                  </div>
                </button>
              );
            })}
      ...
      <div style={{ color:'#1e2a38', fontSize:'0.58rem', fontFamily:"'Courier New',monospace", textAlign:'center', marginTop:'20px', lineHeight:1.5 }}>
        Signals from Open-Meteo · Not financial advice · Verify before trading
      </div>
    </div>
  );
}
